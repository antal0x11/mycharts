'''
    Bar Plot consumer. When it receives a message 
    with csv data (no checks are performed, data have
    been validated on a previus step) and chart title 
    it creates a bar plot chart.

    It stores the created chart temporarily, then sends it 
    to an exchange(name = bar_plot_store) that is responsible 
    to store it.

    It uses bar_plot_q that is binded with bar_plot_charts 
    exchange.
...
def send_to_store(_info : Metachart):
    ...
    channel_s.exchange_declare(exchange="bar_plot_store", exchange_type="fanout")
    channel_s.queue_declare(queue="store_bar_plot_q", durable=True)
    channel_s.queue_bind(queue="store_bar_plot_q", exchange="bar_plot_store")
    
    chart_properties = json.dumps(_info.__dict__)

    props = pika.BasicProperties(headers={"properties" : chart_properties}) 
    
    channel_s.basic_publish(exchange="bar_plot_store", 
                          routing_key="", 
                          body=chart_bytes, 
                          properties=props)
...
if __name__ == "__main__":
    try:
        print(" [+] Bar Plot Worker for myCharts v1.0.0")
        print("")
        main()
